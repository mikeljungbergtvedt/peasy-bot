  ...
  if (!bil.id) {
    ...
    if (pool.length === 0) {
      ...
      await sendTelegram(
        `⚠️ INGEN FINN-KOMPER\n` +
        `${bilLinje}\n\n` +
        (bil.id ? `<a href="${finnUrl}">Apne Finn-sok</a> | <a href="${erpUrl}">Apne i ERP</a>` : `<a href="${finnUrl}">Apne Finn-sok</a>`)
      );
      // Trigg v2 selv om Easy ikke fant Finn-comps — v2 bruker car.info-solgte
      try {
        const v2NoCompsPayload = JSON.stringify({
          registration_number: regnr,
          mileage: bil.mileage || 0,
          id: bil.id,
          easy_eval: null,
          easy_failed_reason: 'no_finn_comps'
        });
        fs.appendFileSync('/Users/bot/peasy-pricing-v2-queue.txt', v2NoCompsPayload + '\n');
        log(`v2 trigget (Easy hadde ingen Finn-comps) for ${regnr}`);
      } catch(eV2) { logErr('v2-trigger uten comps', eV2); }
      return;
    }
    ...
    let anchor = null;
  if (CONFIG.useAiV30) { anchor = await getAnchorAi(pool, seg, bil, vegData); }
  if (!anchor) { anchor = getAnchor(pool, seg); }
    ...
    await sendTelegram(
      formatEvalCard({ ...cardParams, chatPosted }, false),
    ...
    // 11c. Trigger v2 shadow (fire-and-forget, paavirker ikke v18.11)
    // SKIP hvis qaOverrideUrl satt — /finn endrer ikke v2-input, ville gitt duplikat
    try {
      if (qaOverrideUrl) {
        log(`[v18->v2] Skip /finn-trigger for ${regnr} (qaOverride)`);
      } else {
        const v2Payload = JSON.stringify({
          ...
          make: vegData ? vegData.make : (bil.make || ''),
          easy_eval: {
            anker: anchor && anchor.price ? anchor.price : null,
            dLav: valuation ? valuation.dLav : null,
            dHoy: valuation ? valuation.dHoy : null,
            T: valuation ? valuation.T : null,
            margin: valuation ? valuation.margin : null,
            fee: valuation ? valuation.fee : null,
            comps_count: pool ? pool.length : null,
            bracket: valuation ? valuation.bracket : null,
            anchor_reason: anchor && anchor.reason ? anchor.reason : null
          }
        });
        const fs = require('fs');
        fs.appendFileSync('/Users/bot/peasy-pricing-v2-queue.txt', v2Payload + '\n');
        log(`[v18->v2] Triggered v2-shadow for ${regnr}`);
      }
    } catch(e) { log(`[v18->v2] Trigger feilet: ${e.message}`); }
      ...
      for (const update of data.result || []) {
        ...
        if (update.callback_query) {
          ...
          if (cbData.startsWith('confirm:')) {
            ...
            const result = await confirmFinalEstimate(erpIdCb, tok);
            if (result.ok) {
              ...
              await sendTelegram(`❌ Feil for ${erpIdCb}: ${JSON.stringify(result.errors).slice(0,300)}`);
            }
          }
      ...
      if (regnrMatch) {
        ...
        try {
          ...
          try {
            ...
            const kmArg = (regnrMatch[2] && parseInt(regnrMatch[2], 10)) || 0; const bil = { registration_number: regnr, id: null, mileage: kmArg, model_year: null, model_series: null };
  ...
  try {
    ...
    for (const def of LISTE_DEFS) {
      try {
        ...
        if (!res.ok) {
          log(`Liste ${def.nr} (${def.endpoint}): HTTP ${res.status} — endpoint kanskje ikke riktig`);
          continue;
        }
        const data = await res.json();
        const biler = data.data?.data?.data || [];
        log(`Liste ${def.nr} (${def.navn}): ${biler.length} biler`);
        const varsletSet = def.set();
        for (const bil of biler) {
          const regnr = bil.registration_number || '';
          if (!regnr || varsletSet.has(regnr)) continue;
          varsletSet.add(regnr);
          const merke  = bil.drive_no_car_data?.make || bil.make || '';
          const modell = bil.drive_no_car_data?.model_series || bil.model_series || '';
          let melding = `${def.emoji} ${def.navn}\n${regnr} | ${merke} ${modell}`;
          if (def.vis_bud) {
