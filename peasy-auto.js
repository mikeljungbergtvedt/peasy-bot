...
const PAINT_NO = {
  ...
  'BLUE': 'Dypbla metallic', 'RED': 'Rod multi-coat', 'GRAY': 'MiddagsgrÃÂÃÂ¥',
...
async function checkTeslaPrices() {
  ...
  if (alerts.length > 0) {
    ...
    for (const a of alerts) {
      ...
      if (!a.isNew) { msg += 'Senket med ' + fmtNOKstr(a.drop) + ' | FÃÂÃÂ¸r: ' + fmtNOKstr(a.oldPrice) + '\n'; }
      msg += 'Pris nÃÂÃÂ¥: ' + fmtNOKstr(a.finalPrice) + '\n\n';
    }
...
async function checkHeftelser(regNr, page) {
  try {
    ...
    if (text.includes('ingen oppforinger') || text.includes('Ingen oppforinger') || text.includes('ingen oppfÃÂÃÂ¸ringer') || text.includes('Ingen oppfÃÂÃÂ¸ringer')) return 'Ingen heftelser';
...
// ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ FINN URL HELPERS ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
...
function qaCheckComps(car, specs, comps, valuation) {
  if (comps.length < 2) return { approved: false, reason: 'For fÃÂÃÂ¥ Finn-treff (' + comps.length + ')' };
  if (!valuation || !valuation.dLow) return { approved: false, reason: 'Valuation mangler' };
  if (valuation.tEstimate < 10000) return { approved: false, reason: 'T-estimat under 10 000 kr ÃÂ¢ÃÂÃÂ vurder manuelt' };
...
async function searchFinnComps(car, specs, page) {
  ...
  // Then fall back to ÃÂÃÂ±1 year if too few results
  const searches = [
...
async function aiPickAnchor(car, specs, comps) {
  ...
  // Pre-filter to cars within km band ÃÂ¢ÃÂÃÂ Claude only sees comparable cars
  // ÃÂÃÂ±30k first, widen to ÃÂÃÂ±50k, then ÃÂÃÂ±80k if needed
  let pool = [];
  ...
  // Sort by price ASC within pool ÃÂ¢ÃÂÃÂ Claude picks cheapest comparable
  ...
  const prompt = 'Du er en bruktbilekspert i Norge for Peasy (C2B auksjon).\n\n'
    ...
    + 'Sammenlignbare biler pÃÂÃÂ¥ Finn (lignende km, sortert billigst fÃÂÃÂ¸rst):\n'
    + listings + '\n\n'
    + 'Velg den billigste bilen som er et reelt alternativ til vÃÂÃÂ¥r bil. Ignorer ÃÂÃÂ¥penbart feil data.\n'
    + 'Svar KUN med JSON: {"index": N, "price": PRIS, "reason": "en setning pÃÂÃÂ¥ norsk"}';
  ...
  try {
    ...
    console.log('  AI anchor: #' + json.index + ' ÃÂ¢ÃÂÃÂ ' + json.price.toLocaleString('nb-NO') + ' kr | ' + json.reason);
...
async function run(force) {
  ...
  try {
    ...
    for (const car of pendingCars) {
      if (car.hasSdComment) {
        console.log(car.regNr + ' allerede priset (has_sd_comment=1) ÃÂ¢ skip');
        continue;
      }
      console.log('\n' + car.regNr + ' ÃÂ¢ÃÂÃÂ ' + car.make + ' ' + car.model + ' ' + car.year + ' ' + car.km + 'km');
      try {
        ...
        console.log('  D lav: ' + fmtNOKstr(valuation.dLow) + ' | D hÃÂÃÂ¸y: ' + fmtNOKstr(valuation.dHigh));
        let sdComment = null;
      ...
      for (const update of data.result || []) {
        ...
        if (text && text.startsWith('/finn ')) {
          ...
          if (regNr) {
            ...
            if (regNr && !erpCar) { await sendTelegram('ÃÂ¢ÃÂÃÂ ÃÂ¯ÃÂ¸ÃÂ ' + regNr + ' ikke funnet i ERP-koen. Bilen er allerede behandlet eller ikke registrert.'); return; }
          ...
          try {
            ...
            if (comps.length === 0) {
              ...
              let reply = 'ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ\n';
              reply += 'ÃÂ°ÃÂÃÂÃÂ <b>' + (regNr || '') + ' ÃÂ¢ÃÂÃÂ ' + (carInfo?.make || '') + ' ' + (carInfo?.model || '') + ' ' + carYear + '</b>\n';
              reply += (carKm ? carKm.toLocaleString('nb-NO') : '0') + 'km | ' + (carInfo?.fuel || '') + ' | ' + (carInfo?.gearbox || '') + ' | ' + (carInfo?.drive || '') + ' | ' + hk + 'hk\n';
              reply += 'ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ\n\n';
              reply += 'ÃÂ°ÃÂÃÂÃÂ <b>FINN-SÃÂÃÂK (MANUELL)</b>\n';
              reply += comps.length + ' treff | <a href="' + finnUrl + '">ÃÂÃÂpne sÃÂÃÂ¸k</a>\n';
              reply += 'ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ\n\n';
              reply += 'ÃÂ°ÃÂÃÂÃÂ <b>FINN KOMPS</b>\n';
              top5.forEach((c, i) => {
                const isAnker = anchor && c.price === anchor.price && c.km === anchor.km;
                reply += (i + 1) + '. ' + c.price.toLocaleString('nb-NO') + ' kr | ' + c.km.toLocaleString('nb-NO') + 'km | ' + c.year + (isAnker ? ' ÃÂ¢ÃÂÃÂ anker' : '') + '\n';
              });
              reply += 'Snitt: <b>' + fmtNOKstr(avg) + '</b>\n';
              if (anchor && anchor.aiReason) reply += 'ÃÂ°ÃÂÃÂ¤ÃÂ ' + anchor.aiReason + '\n';
              reply += 'ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ\n\n';
              reply += 'ÃÂ°ÃÂÃÂÃÂ° <b>KALKYLE</b>\n';
              reply += 'Finn anker:       <b>' + lowest.toLocaleString('nb-NO') + ' kr</b>\n';
              reply += 'ÃÂÃÂ 0.88 (T est):  ' + val.tEstimate.toLocaleString('nb-NO') + ' kr\n';
              reply += 'Peasy fee:        ' + val.fee.toLocaleString('nb-NO') + ' kr\n';
              reply += 'Selger T:         ' + val.sellerT.toLocaleString('nb-NO') + ' kr\n';
              reply += '<b>D lav: ' + val.dLow.toLocaleString('nb-NO') + ' ÃÂ¢ÃÂÃÂ D hÃÂÃÂ¸y: ' + val.dHigh.toLocaleString('nb-NO') + ' kr</b>\n\n';
              if (finnListing) reply += 'Finn-annonse: ÃÂ¢ÃÂÃÂ <a href="https://www.finn.no/mobility/search/car?q=' + regNr + '">' + finnListing.price.toLocaleString('nb-NO') + ' kr (' + finnListing.km.toLocaleString('nb-NO') + ' km)</a>\n';

              else reply += 'Finn-annonse: ÃÂ¢ÃÂÃÂ Ikke funnet\n';
              if (heftelser) reply += 'Heftelser: ' + heftelser + '\n';
              if (erpCar?.comment) reply += 'Kundekommentar: ' + erpCar.comment.substring(0, 300) + '\n';
              reply += 'ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ\n\n';
              reply += 'ÃÂ°ÃÂÃÂÃÂ <b>ERP</b>\n';
              reply += 'ÃÂ¢ÃÂÃÂ ÃÂ¯ÃÂ¸ÃÂ Ikke skrevet ÃÂ¢ÃÂÃÂ manuell gjennomgang\n';
