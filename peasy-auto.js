...
function formatSingleResult(r) {
  ...
  if (r.finnListing) {
    ...
    const gapStr = gap >= 0 ? '+' + Math.round(gap/1000) + 'k over anker' : Math.abs(Math.round(gap/1000)) + 'k under anker';
    msg += 'Finn-annonse: ✅ <a href="https://www.finn.no/mobility/search/car?q=' + car.regNr + '">' + r.finnListing.price.toLocaleString('nb-NO') + ' kr (' + gapStr + ')</a>\n';
  } else {
  ...
  if (r.finnListing)           msg += '⚠️ Ikke skrevet — bil annonsert pa Finn\n';
  else if (qa && !qa.approved) msg += '🚨 FLAGGET — ikke skrevet\nQA: ' + qa.reason + '\n';
  else if (hasHeft)            msg += '✅ Skrevet til ERP (heftelser — sjekk Finans)\n';
