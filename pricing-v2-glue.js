...
// AI-kallet skal se NOEYAKTIG samme comp-objekter som standalone-v2.
// _spec/_comparable/_flag er kun for kort-visning — sendes de med, ser
// Sonnet "ikke sammenlignbar" paa compsene og nekter aa sette anker.
...
function comparability(origin, spec) {
  ...
  // EV: Finn-titler bruker overboost-hk, car.info kontinuerlig — gi videre toleranse
...
async function runV2Pricing(regnr, km) {
  ...
  // Ankergrunnlag: KUN sammenlignbare FORHANDLER-salg. car.info sine "privat"-priser
  // er syntetiske kopier ~5-8% under dealer og drar ankeret kunstig ned. Privat
  // brukes bare som backup hvis < 4 forhandler-comps.
  ...
  let relaxed = '';
  if (anchorPool.length < 4) { anchorPool = comparablePool; relaxed = 'privat inkludert som backup'; }
  if (anchorPool.length < 4) { anchorPool = deduped; relaxed = 'relaksert: alle comps'; }
  const capN = parseInt(process.env.PEASY_V2_MAX_COMPS, 10) || 25;
  const comps = capComps(anchorPool, kmNum, capN);
  console.log(`[v2-glue] ${regnr} sammenlignbare ${comparablePool.length}/${deduped.length} | forhandler ${dealerPool.length}`
    + (relaxed ? ` (${relaxed})` : '') + ` -> anker-pool ${comps.length}`);
  ...
  let anchor = null;
  if (comps.length > 0) {
    anchor = await chooseAnchor({ data, origin, comps: stripMeta(comps) });
    // Sikkerhetsnett: gir gatet pool ikke anker, kjor en gang til paa HELE
    // det ugatede settet — da er input identisk med standalone-v2 og inline
    // kan per definisjon ikke feile der standalone lykkes.
    if (!ankerOk(anchor) && anchorPool !== deduped && deduped.length > 0) {
      console.log(`[v2-glue] ${regnr} gatet pool ga ikke anker -> retry med alle ${deduped.length} comps (som standalone v2)`);
